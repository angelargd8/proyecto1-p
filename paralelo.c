...
#define DIFF_ITERS      1        // sube/ baja para más/menos carga
#define USE_GAUSS_BLUR  1        // 1=ON, 0=OFF
...
static void drawGridCycle(
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            ...
            // LATCH: cuando el pulso en esta celda ya terminó (subida + bajada)
            // Fin del pulso local = start_t + 2*FADE_TIMEss
            if (stage_time >= start_t + 0.4f*FADE_TIME && lastLatchedStage[r][c] != global_stage_idx) {
...
int main(int argc, char**argv){
    ...
    while (running) {
        ...
        // Etapas (duración completa del pulso local: subida + bajada)
